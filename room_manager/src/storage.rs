...
const SIGNALING_INSTANCE_LOAD_KEY: &str = "signaling:instance_load";
const SIGNALING_ROOM_PREFIX: &str = "signaling:room:";
const SIGNALING_SESSION_PREFIX: &str = "signaling:session:";
    ...
    with_op_timeout(async move {
        ...
        loop {
            let (next, keys): (u64, Vec<String>) = redis::cmd("SCAN")
                .arg(cursor)
                .arg("MATCH")
                .arg(format!("{ROOM_PREFIX}*:binding"))
                .arg("COUNT")
                .arg(200)
                .query_async(&mut conn)
                .await?;
            for key in keys {
                if let Some(rest) = key.strip_prefix(ROOM_PREFIX) {
...
pub async fn delete_signaling_room_members(redis: &redis::Client, room_id: &str) -> Result<()> {
    with_op_timeout(async move {
        let mut conn = redis.get_connection_manager().await?;
        let key = format!("{SIGNALING_ROOM_PREFIX}{room_id}:members");
...
pub async fn delete_signaling_session(redis: &redis::Client, session_id: &str) -> Result<()> {
    with_op_timeout(async move {
        let mut conn = redis.get_connection_manager().await?;
        let key = format!("{SIGNALING_SESSION_PREFIX}{session_id}");
        let _: usize = conn.del(key).await?;
        Ok(())
    })
    .await
